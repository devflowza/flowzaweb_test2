...
export function FinanceShowcase() {
  ...
  return (
    <Section tone="ink" className="">
      <Container>
        <SectionHeading
          eyebrow={FINANCE_SHOWCASE.badge}
        ...
        >
            ...
            >
              {FINANCE_SCREENSHOTS.map((shot, i) => (
                <button
                  ...
                  onClick={() => select(i)}
                  className={cn(
                    "relative shrink-0 snap-start overflow-hidden rounded-xl border px-4 py-3 text-left transition-all duration-400 ease-(--ease-1)",
                    i === active
                ...
                >
                  ...
                  <span className="mt-0.5 block text-[0.66rem] uppercase tracking-[0.1em] text-white/65">
                    {shot.sub}
                  </span>
                  {i === active && autoplaying ? (
                    <span
                      ...
                      aria-hidden="true"
                      className="absolute inset-x-0 bottom-0 h-0.5 origin-left bg-gradient-to-r from-accent to-brand-500"
          ...
          <Reveal delay={0.1} className="order-1 lg:order-2">
            <div className="shadow-(--shadow-mega)">
              <div className="overflow-hidden rounded-card border border-white/10 bg-ink">
                ...
                <div className="flex items-center gap-3 border-b border-white/10 bg-white/[0.04] px-4 py-2.5">
                  ...
                  <span className="mx-auto flex items-center gap-1.5 rounded-full bg-white/5 px-4 py-1 text-[0.7rem] text-white/65">
                    {FINANCE_SHOWCASE.urlBar} · {current.label}
                  </span>
                </div>
                {/* Screens — every panel element is rendered so each tab's
                    aria-controls resolves, but only the active panel and its two
                    neighbours mount an <Image>, so the network cost is unchanged. */}
                  ...
                  {FINANCE_SCREENSHOTS.map((shot, i) => {
                    ...
                    return (
                      <div
                        ...
                        tabIndex={i === active ? 0 : -1}
                        className={cn(
                          "absolute inset-0 transition-opacity duration-700 ease-(--ease-1)",
            ...
            <div className="mt-5 flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <ul className="mt-2 hidden flex-wrap gap-x-4 gap-y-1 sm:flex">
                  {current.bullets.map((b) => (
                    <li
                      key={b}
                      className="flex items-center gap-1.5 text-[0.68rem] uppercase tracking-[0.08em] text-white/65"
                    >
              ...
              <div className="flex shrink-0 gap-2">
                <button
                  ...
                  className="flex size-10 items-center justify-center rounded-full border border-white/15 bg-white/5 text-white/85 transition-all duration-300 ease-(--ease-1) hover:border-white/35 hover:text-white"
                >
                  {playing ? (
