 ...
 * no photography of their own (about, contact, pricing, locations, get-started,
...
/**
 * /pricing — what a year on each billing period actually costs.
 *
 * Deliberately not a drawing of the three plan cards: the real plan grid renders
 * directly under this banner, so repeating it would be an illustration of the
 * component beside it. The yearly saving and the free-trial runway are the parts
 * of the pricing story the grid doesn't show.
 */
function pricingValue(w, h) {
  const r = rng(5);
  const px = w * 0.075;
  const chartW = w * 0.53;
  const top = h * 0.17;
  ...
  /* Twelve months, monthly billing against yearly billing */
  const months = 12;
  const slot = (chartW - 76) / months;
  const barW = slot * 0.3;
  const plotH = cardH - 150;
  const plotY = top + 96;

  const bars = many(months, (i) => {
    const x = px + 38 + i * slot + slot * 0.1;
    const full = plotH * (0.62 + r() * 0.34);
    return (
      rect({
        x: round(x),
        y: round(plotY + plotH - full),
        width: round(barW),
        height: round(full),
        rx: round(barW / 2),
        fill: P.line,
      ...
      rect({
        x: round(x + barW + 5),
        y: round(plotY + plotH - full * 0.75),
        width: round(barW),
        height: round(full * 0.75),
        rx: round(barW / 2),
        fill: "url(#brand)",
      })
    );
  ...
  /* Savings ring */
  const ringCx = w * 0.775;
  const ringCy = top + cardH * 0.42;
  const ringR = 78;
  const circumference = 2 * Math.PI * ringR;
  ...
  return (
    backdrop(w, h, { leak: "bottom-left" }) +
    /* Billing comparison */
    card({ x: px, y: top, w: chartW, h: cardH, r: 28, shadow: "shadowLift" }) +
    rect({
      x: px + 38,
      y: top + 34,
      width: 176,
    ...
    /* Legend: monthly vs yearly */
    group(
      {},
      rect({
        x: round(px + chartW - 220),
        y: round(top + 32),
        width: 14,
        height: 14,
        rx: 4,
    ...
    many(4, (i) =>
      line({
        x1: round(px + 38),
        y1: round(plotY + (plotH / 3) * i),
        x2: round(px + chartW - 38),
        y2: round(plotY + (plotH / 3) * i),
        stroke: P.line,
        "stroke-width": 1,
        "stroke-dasharray": i === 3 ? undefined : "4 7",
      }),
    ) +
    bars +
    /* Saving */
    card({ x: round(w * 0.635), y: top, w: round(w * 0.29), h: cardH, r: 28 }) +
    circle({
      cx: round(ringCx),
      cy: round(ringCy),
      r: ringR,
    ...
    circle({
      ...
      "stroke-width": 16,
      "stroke-linecap": "round",
      "stroke-dasharray": `${round(circumference * 0.25)} ${round(circumference)}`,
      transform: `rotate(-90 ${round(ringCx)} ${round(ringCy)})`,
    }) +
    rect({
      x: round(ringCx - 44),
    ...
    /* Monthly vs yearly totals under the ring */
    many(2, (i) => {
      const y = ringCy + ringR + 44 + i * 40;
      return (
        rect({ x: round(w * 0.66), y: round(y), width: 74, height: 10, rx: 5, fill: P.line }) +
        rect({
          x: round(w * 0.66 + 90),
          y: round(y - 3),
          width: round(w * (i === 0 ? 0.13 : 0.095)),
          height: 16,
          rx: 8,
          fill: i === 0 ? P.graySoft : "url(#brand)",
          opacity: i === 0 ? 0.55 : 1,
        })
      );
    }) +
    /* Free-trial runway, overlapping both cards */
    group(
      {},
      card({
        x: round(px + chartW - 150),
        y: round(top + cardH - 42),
        w: 330,
        h: 92,
        r: 22,
        shadow: "shadowChip",
        ...
        iconTile({
          x: round(px + chartW - 124),
          y: round(top + cardH - 17),
          size: 42,
          glyph: "clock",
        }) +
        rect({
          x: round(px + chartW - 66),
          y: round(top + cardH - 8),
          width: 128,
          height: 10,
          rx: 5,
    ...
    floatingChip({ x: w * 0.03, y: h * 0.055, w: 236, h: 78, glyph: "shield" })
  );
}
...
const SCENES = [
  ...
  { out: "pricing.webp", size: BANNER, draw: pricingValue },
  {
